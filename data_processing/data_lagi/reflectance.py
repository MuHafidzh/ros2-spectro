...
def process_reflectance_data(reflectance_data):
    ...
    for mdg_dir, files in reflectance_data.items():
        ...
        for file_num, reflectance_df in files.items():
            ...
            # 2. First smoothing
            smoothed = smooth_data(corrected, window=31)

            # Absorbance = log(1/(0.01*col(X)))
            # col(X) adalah data reflectance yang akan ditransformasi
            absorbance = np.log10(1 / (0.01 * smoothed))  # Add small value to avoid log(0)
            
            # 3. First derivative with smoothing
            # first_deriv = calculate_derivative(absorbance, order=1)
            ...
            # third_deriv = calculate_derivative(second_deriv, order=1)
            # fourth_deriv = calculate_derivative(third_deriv, order=1)
            
            # 5. Normalize to [0, 1]
            normalized = normalize_data(second_deriv)
            # normalized = normalize_data(fourth_deriv)
...
def average_and_plot_results(processed_data):
    ...
    #                 color=colors[i % len(colors)], alpha=0.3 if file_num > 1 else 0.7, linewidth=1)
        
    #     # Plot average of absorbance
    #     if files:
    #         all_absorbance = [data['absorbance'] for data in files.values()]
    #         avg_absorbance = np.mean(all_absorbance, axis=0)
    #         wavelength = list(files.values())[0]['wavelength']
    #         plt.plot(wavelength, avg_absorbance, color=colors[i % len(colors)], 
    #                 linewidth=3, label=f'{mdg_dir} (avg)')  # Make avg line thicker
    # plt.xlabel('Wavelength (nm)')
